    ...
    <>
      <SimpleAppBar position="fixed" placementFixed>
        ...
        <Box sx={{ display: "flex", justifyContent: "flex-end", alignItems: "center", gap: "1rem" }}>
          <LogIn className={styles.home_nav_login} color='var(--mui-palette-primary-contrastText)' onClick={() => router.push('/auth')} />
          <ThemeSwitchButton className={styles.home_nav_theme_switch} isDarkMode={isDarkMode} toggleTheme={() => dispatch(toggleTheme())} color='var(--mui-palette-primary-contrastText)' />
        </Box>
      </SimpleAppBar>
      <Grid className={styles.home_grid} container spacing={3}>
